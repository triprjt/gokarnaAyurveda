            ...
            <nav className="w-full px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    ...
                    <div className="hidden md:flex items-center space-x-8">
                        <Link href="#home" className={`${isHeroVisible ? 'text-white' : 'text-gray-700'} hover:text-emerald-700 transition-colors`}>
                            Home
                        </Link>
                        <Link href="#services" className={`${isHeroVisible ? 'text-white' : 'text-gray-700'} hover:text-emerald-700 transition-colors`}>
                            Services
                        </Link>
                        <Link href="#testimonials" className={`${isHeroVisible ? 'text-white' : 'text-gray-700'} hover:text-emerald-700 transition-colors`}>
                            Testimonials
                        </Link>
                        <Link href="#contact" className={`${isHeroVisible ? 'text-white' : 'text-gray-700'} hover:text-emerald-700 transition-colors`}>
                            Contact
                        </Link>
                    ...
                    <button className={`md:hidden ${isHeroVisible ? 'text-white' : 'text-gray-700'} hover:text-emerald-700 transition-colors`} onClick={() => toggleMenu()}>
                        {!isMenuOpen ? <svg className="w-6 h-6" fill="none" stroke={isHeroVisible ? 'white' : 'black'} viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
                        </svg> : <svg className="w-6 h-6" fill="none" stroke={isHeroVisible ? 'white' : 'black'} viewBox="0 0 24 24">
